...
def _final_add(job: dict[str, str]) -> dict[str, Any]:
    ...
    updated = memory.update_tool_adoption_job(job["id"], status="added_to_universal_ai")
    memory.update_tool_scout_status(report["id"], "built")
    return {"status": "added_to_universal_ai", "job": updated, "report": report, "skill_id": skill_id}
