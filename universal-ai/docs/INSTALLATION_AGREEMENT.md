# Universal AI Installation Agreement

Read this before installing Universal AI / Sekhar AI OS.

## Local-first and free-first

Universal AI is intended to run as a local-first assistant using fully free/open-source/local-first tools where practical. Paid, freemium, trial-limited, reward-network, closed, or privacy-risky tools must not be added to the seven-project core. When a paid or restricted tool is mentioned, Universal AI should search for a fully free alternative and add only the replacement if it improves a project.

## System access

Universal AI may inspect this machine's basic specifications, such as CPU count, RAM, operating system, and installed local capability status, so it can choose work that fits this system. Universal AI should run tasks within the capability of the current machine and delegate heavier work only to approved paired systems.

Universal AI must not perform destructive actions, installs, remote shell actions, remote desktop control, credential access, or sensitive file operations without approval.

## Multi-system cluster mode

Universal AI may support a multi-system mode where several trusted machines run together as one assistant. This mode is designed for owner-approved systems only.

By accepting this agreement, you acknowledge:

- This installation may include code paths for future local cluster mode.
- A machine may auto-connect to other Universal AI nodes only after explicit pairing/approval on the involved systems.
- After a system is installed, paired, and approved, Universal AI may automatically reconnect to that paired system when a task needs its capability, run approved non-sensitive worker processes, return results to the manager, and disconnect or idle the worker when the task is complete.
- Universal AI may connect to and use multiple already-paired systems at the same time when the manager has work that benefits from several workers and each worker stays within its capability limits.
- Approved self-improvement data should be published to the Universal AI Git repository as an improvement proposal, including approved/trusted skills, tool rules, free-tool replacements, capability profiles, sanitized reflection lessons, and test summaries.
- Every Universal AI system, paired or unpaired, may periodically check the Git repository for shared improvement proposals, with a default interval of 24 hours. When new improvement data is found, the system should verify it, check local compatibility and safety policy, then ask the local user or manager for approval before adopting it.
- Universal AI should not directly push improvement data to other systems. Systems receive shared improvements by Git clone/fetch/pull/install.
...
- Universal AI must not silently join, monitor, control, or run workloads on systems where the owner/admin has not installed and approved it.
