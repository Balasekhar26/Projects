...
from installer import setup_kattappa
...
def test_installation_agreement_mentions_cluster_consent() -> None:
    text = setup_kattappa.INSTALLATION_AGREEMENT.read_text(encoding="utf-8")
    ...
    assert "published to the Kattappa AI OS Git repository" in text
...
def test_installer_writes_cluster_safety_defaults(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(setup_kattappa, "BACKEND_ENV", env_path)
    profile = setup_kattappa.MachineProfile(
        tier="minimum",
        cpu_logical=4,
        ram_gb=8.0,
        fast="qwen2.5:0.5b",
    ...
    setup_kattappa.write_backend_env(profile)
    text = env_path.read_text(encoding="utf-8")
    assert "KATTAPPA_CLUSTER_ENABLED=false" in text
    assert "KATTAPPA_CLUSTER_PAIRING_REQUIRED=true" in text
    assert "KATTAPPA_REMOTE_ACTIONS_NEED_APPROVAL=true" in text
    assert "KATTAPPA_UNPAIRED_IMPROVEMENT_CHECK_ENABLED=true" in text
