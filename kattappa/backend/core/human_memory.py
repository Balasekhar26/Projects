...
class RecallEngine:
    ...
    def recall(
        ...
        *,
        ...
        now = time.time()
        hits: list[RecallHit] = []
        for record in HumanMemoryStore.all_records(include_pending=False):
...
class HumanMemory:
    ...
    def ingest(
        ...
        *,
        ...
        # Trust defaults: user / system are trusted; web / screen / file are not.
        if trusted is None:
            trusted = source in {"user", "system", "voice"}

        if not _tokens(text):
            return IngestResult(False, StoreDecision.FORGET, False, False, None,
                                ImportanceScore(0, 0, 0, 0, 0, 0, StoreDecision.FORGET),
                                ["empty or contentless event"])

        # Sensory layer: de-duplicate before anything else.
        if SensoryDeduplicator.is_duplicate(text):
            reasons.append("near-duplicate sensory event discarded")
            return IngestResult(False, StoreDecision.FORGET, True, False, None,
                                ImportanceScore(0, 0, 0, 0, 0, 0, StoreDecision.FORGET), reasons)

        # Working memory always observes the event.
        ...
        if score.decision is StoreDecision.FORGET:
            ...
            return IngestResult(False, score.decision, False, False, None, score, reasons)

        mem_type = classify_memory_type(text)

        # Untrusted isolation: external content may not enter long-term
        # semantic/procedural memory without explicit approval.
        pending = False
