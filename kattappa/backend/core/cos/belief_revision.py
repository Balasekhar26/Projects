...
from backend.core.cos.state_representation import BeliefState, BeliefStatus, Evidence, PropertyValue
...
class BeliefRevisionEngine:
    ...
    def __init__(self, belief_engine: BeliefEngine, tms: Optional[TruthMaintenanceSystem] = None):
        self.belief_engine = belief_engine
        self.tms = tms if tms is not None else TruthMaintenanceSystem(JustificationManager())
    ...
    def expand(self, entity_uuid: str, prop_name: str, prop_val: PropertyValue, reason: str = "Expansion assertion") -> RevisionRecord:
        ...
        if prior is not None:
            raise ValueError(f"Property {entity_uuid}.{prop_name} already exists. Cannot expand.")
        ...
        just = Justification(
            ...
            supporting_evidence_ids=[e.evidence_id for e in expanded_pv.evidence_history] if expanded_pv.evidence_history else [],
        ...
        log_event("agm_expansion", f"Asserted fresh belief {entity_uuid}.{prop_name} = '{expanded_pv.value}'")
        return record
    ...
    def revise(
        ...
        self.belief_engine.dependency_tracker.propagate_change(self.belief_engine.state, entity_uuid, prop_name)
        self.tms.propagate_justifications(self.belief_engine.state, self.belief_engine.dependency_tracker.dependencies)
    ...
    def contract(self, entity_uuid: str, prop_name: str, reason: str = "Contraction request") -> Optional[RevisionRecord]:
        ...
        self.revision_history.append(record)
        log_event("agm_contraction", f"Contracted belief {entity_uuid}.{prop_name} (status=RETRACTED)")
        return record
