...
class ObsidianMemory:
    ...
    def write_daily_note(self, content: str, category: str = "general") -> Path:
        """Appends log text to the daily note markdown file."""
    ...
    def write_concept_page(self, title: str, content: str, tags: List[str] = None, connections: List[str] = None) -> Path:
        """Creates or overwrites a concept markdown file."""
    ...
    def write_project_note(self, project_name: str, key: str, value: str, section: str = "Updates") -> Path:
        """Appends or updates a section in a project markdown file."""
        sanitized_name = self._sanitize_filename(project_name)
        project_path = self.vault_path / "Projects" / f"{sanitized_name}.md"
        ...
        if not project_path.exists():
            ...
            project_path.write_text(header + entry, encoding="utf-8")
        else:
