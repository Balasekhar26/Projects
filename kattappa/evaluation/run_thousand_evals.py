#!/usr/bin/env python
"""1000 Tasks Evaluation Runner — Kattappa.

Runs 1000 tasks from ``evaluation/thousand_tasks.yaml`` through the live app via
FastAPI TestClient and verifies assertions.

Usage::

    # Run all 1000 tasks
    python evaluation/run_thousand_evals.py

    # Run with a limit (e.g. first 10 tasks)
    python evaluation/run_thousand_evals.py --limit 10

    # Run a specific category
    python evaluation/run_thousand_evals.py --category "Coding & Debugging"
"""
...
os.environ["KATTAPPA_BYPASS_APPROVAL"] = "1"

# Create a clean temp sandbox directory for memory and database isolation
tmp_dir = Path(tempfile.mkdtemp(prefix="kattappa_thousand_eval_"))
...
YAML_PATH = EVAL_DIR / "thousand_tasks.yaml"
SCORECARD_PATH = EVAL_DIR / "thousand_scorecard.json"
HISTORY_PATH = EVAL_DIR / "thousand_history.jsonl"
...
def run_evaluation(limit: int | None = None, category_filter: str | None = None) -> dict:
    ...
    run_label = f"1000 TASKS"
    if category_filter:
        run_label += f" — {category_filter}"
    if limit:
        run_label += f" (Limit {limit})"

    print(f"\nLoaded {len(tasks)} tasks ({run_label})\n")
    print("=" * 60)
    print(f"RUNNING KATTAPPA 1000 TASKS EVALUATION HARNESS")
    print("=" * 60)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Kattappa 1000 Tasks Evaluation Harness")
    ...
    try:
        scorecard = run_evaluation(limit=args.limit, category_filter=args.category)
    finally:
