...
- Legacy/reference system: `ai_system` and `classic-kattappa-ai-system`
...
- There are two overlapping systems: the current `backend` system and the older `ai_system`/`classic-kattappa-ai-system` code. That increases maintenance load.
...
8. Move legacy `ai_system` and `classic-kattappa-ai-system` behind docs/reference status or archive them once no launcher depends on them.
