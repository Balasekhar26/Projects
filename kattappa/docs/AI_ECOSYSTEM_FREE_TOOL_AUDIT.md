...
Machine-readable registry: `kattappa/config/ai_ecosystem_topic_audit.json`.
Developer-toolbox registry: `kattappa/config/developer_toolbox_audit.json`.
...
The exact 34-topic decision table is tracked in `kattappa/config/ai_ecosystem_topic_audit.json` and exposed through `/free-tools` under `ai_ecosystem_topic_audit`. That registry is the canonical machine-readable source for Kattappa's future tool-scouting and self-improvement checks.

## Developer Toolbox Pass

The later 7-tool batch is tracked separately in `kattappa/config/developer_toolbox_audit.json` and exposed through `/free-tools` under `developer_toolbox_audit`.
