...
def verify_source_provenance() -> tuple[bool, str, list[str]]:
    ...
    try:
        ...
        for line in status_lines:
            ...
            # Ignore runtime evidence artifacts and evaluation reflections
...
def run_full_sharded_suite(shard_size: int = 250):
    ...
    if not valid_start:
        ...
        with open(evidence_dir / "release-verdict.json", "w", encoding="utf-8") as f:
            json.dump(release_verdict, f, indent=2)
        return 3
    ...
    changed_data_paths = [p for p in set(list(pre_digest.keys()) + list(post_digest.keys())) if pre_digest.get(p) != post_digest.get(p)]
    ...
    test_verdict_data = aggregate_results(evidence_dir)
    ...
    # SOLE author of release-verdict.json
    with open(evidence_dir / "release-verdict.json", "w", encoding="utf-8") as f:
        json.dump(release_verdict, f, indent=2)
