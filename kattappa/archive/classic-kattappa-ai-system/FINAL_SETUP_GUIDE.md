# 🔥 KATTAPPA AI SYSTEM — FINAL SETUP GUIDE
...
Open PowerShell in the `kattappa-ai-system` folder and run:
...
```
kattappa-ai-system/RUN_DESKTOP_APP.bat
```
...
```powershell
python kattappa_ai_system.py --once "explain quantum computing basics"
```

Run one coding task:
```powershell
python kattappa_ai_system.py --code "create a python calculator"
```

Search the web:
```powershell
python kattappa_ai_system.py --search "latest AI trends 2026"
```
...
```
kattappa-ai-system/
├── kattappa_ai_system.py       ← Main program (DO NOT EDIT)
...
```powershell
python kattappa_ai_system.py --provider ollama
```
...
```powershell
python kattappa_ai_system.py --debug
```
