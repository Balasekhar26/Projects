...
class FirstRunWizard extends EventEmitter {
  ...
  async stepVoiceProfile() {
    ...
    this.emit("info", "Three voice options available:");
    this.emit("info", "  1. System voices (always available, basic quality)");
    this.emit("info", "  2. OpenAI TTS (online, high quality)");
    this.emit("info", "  3. XTTS voice clone (offline, custom voice)");
