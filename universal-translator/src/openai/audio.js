...
const BUILTIN_TTS_VOICES = ["alloy", "ash", "ballad", "coral", "echo", "fable", "onyx", "nova", "sage", "shimmer", "verse", "marin", "cedar"];

function getConfig() {
  return getCoreConfig();
}

function getVoiceStorePath() {
  return path.join(getConfig().tempDir, "openai-custom-voices.json");
}
...
function getAuthorizationHeaders(extraHeaders = {}) {
  const config = assertOpenAiApiKey();
  return {
...
async function readStoredCustomVoices() {
  try {
    const raw = await fs.readFile(getVoiceStorePath(), "utf8");
...
async function writeStoredCustomVoices(voices) {
  const voiceStorePath = getVoiceStorePath();
...
async function synthesizeSpeech({ input, voice, instructions, language, speed, format = "wav" }) {
  const config = assertOpenAiApiKey();
